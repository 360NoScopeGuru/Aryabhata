...
import { applySlashCommand, findSlashMatches, type SlashCommand } from '@/lib/slashCommands'
import { getActiveModel, isBlendMode, MIXING_MODELS, useAppStore } from '@/store/appStore'
...
export default function ChatInput({
  ...
  // Reset slash selection when the query changes — adjusted during render
  ...
  const handleKey = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (slashActive && slashMatches.length > 0) {
      ...
      if (
        e.key === 'Tab' ||
        (e.key === 'Enter' && slashMatches.length > 0 && value.split(/\s+/).length === 1)
      ) {
  ...
  const handleSend = () => {
    ...
    if (slashActive && slashMatches.length > 0) {
      const exact = slashMatches.find(
        (c) => value.toLowerCase().startsWith(c.trigger + ' ') || value.toLowerCase() === c.trigger,
      )
      if (exact) {
        pickSlash(exact)
        return
  ...
  const canSend =
    (value.trim() || pastedImage || attachedFile) && !disabled && !streaming && !noModel
  const blend = isBlendMode(selectedModels)
  const activeModelId = mode !== 'image' ? getActiveModel(selectedModels) : null
  const activeModel =
    activeModelId && !noModel ? MIXING_MODELS.find((m) => m.id === activeModelId) : null
        ...
        <div className="composer-head">
          ...
          {pastedImage && (
            <span
              ...
              style={{ color: 'var(--accent2)', borderColor: 'rgba(255,212,122,.3)' }}
            >
              Image attached
            </span>
          )}
            ...
            {promptsOpen && (
              <div className="prompt-library-popover">
                <div className="pl-header">
                  ...
                  {value.trim() && (
                    <button className="pl-save-btn" onClick={() => setSaving((s) => !s)}>
                      + Save current
                    </button>
                  )}
                </div>
          ...
          >
            {value.length > 0 && (
              <span style={{ color: 'var(--ink-dim)', marginRight: '6px' }}>
                ~{estimatedTokens} tok
              </span>
            )}
        ...
        <textarea
          ...
          value={
            voice.listening && voice.interim
              ? value + (value && !value.endsWith(' ') ? ' ' : '') + voice.interim
              : value
          }
