...
export default function PersonaGallery({ systemPrompt, onSelect }: Props) {
  ...
  // Active card is derived from systemPrompt rather than tracked separately,
  // so it stays in sync automatically when the textarea is edited manually.
  const activeId = allPersonas.find((p) => p.prompt === systemPrompt)?.id ?? null
  ...
  return (
    <div className="persona-gallery">
      <div className="persona-scroll">
        ...
        <button
          className="persona-card persona-add-btn"
