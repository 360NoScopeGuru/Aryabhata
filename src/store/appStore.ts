...
> = {
  precise: { temperature: 0.1, topP: 0.9, topK: 10 },
...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      appendToLastMessage: (convId, delta) =>
        set((s) => {
      ...
      updateLastMessageTelemetry: (convId, fields) =>
        set((s) => {
          const msgs = [...(s.messages[convId] || [])]
          if (msgs.length === 0) return s
      ...
      removeCustomPersona: (id) =>
        set((s) => ({ customPersonas: s.customPersonas.filter((p) => p.id !== id) })),
      ...
      truncateMessagesFrom: (convId, index) =>
        set((s) => ({
