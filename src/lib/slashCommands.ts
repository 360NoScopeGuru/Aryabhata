...
export const SLASH_COMMANDS: SlashCommand[] = [
  // ── Transform commands ─────────────────────────────────────────────
  {
    ...
    kind: 'transform',
    transform: (t) => `Explain this like I'm five years old, using simple words and analogies. No jargon.\n\nQuestion: ${t}`,
  },
  {
    trigger: '/critique',
    ...
    kind: 'transform',
    transform: (t) => `Critique this rigorously. Find every weakness, flaw, anti-pattern, missed optimization, or logical gap. Be sharp but constructive.\n\n${t}`,
  },
  {
    trigger: '/summarize',
    label: 'Summarize',
    ...
    kind: 'transform',
    transform: (t) => `Summarize the following in 3-5 bullet points, leading with the most important takeaway:\n\n${t}`,
  },
  {
    trigger: '/translate',
    label: 'Translate',
    ...
    transform: (t) => {
      ...
      if (match) return `Translate the following to ${match[1]}. Provide only the translation, no explanation.\n\n${match[2]}`
  ...
  },
  {
    ...
    kind: 'transform',
    transform: (t) => `Write production-quality code for the following. Include brief inline comments only where the logic is non-obvious.\n\nRequirement: ${t}`,
  },
  {
    trigger: '/expand',
    label: 'Expand',
    ...
    kind: 'transform',
    transform: (t) => `Expand the following note into a complete, well-structured response. Preserve the original intent, add depth, examples, and context.\n\nNote: ${t}`,
  },
  {
    trigger: '/refactor',
    label: 'Refactor',
    ...
    kind: 'transform',
    transform: (t) => `Refactor this code for clarity, modern idioms, and minimal complexity. Preserve behavior. Show the refactored version with a brief summary of what changed.\n\n${t}`,
  },
  {
    trigger: '/whatif',
    label: 'What If…',
    ...
    kind: 'transform',
    transform: (t) => `Explore counterfactuals for the following. What are 3-5 plausible alternative outcomes or scenarios? Reason about each.\n\n${t}`,
  },
...
export function applySlashCommand(input: string, cmd: SlashCommand): { kind: SlashKind; payload: string; actionKey?: string } {
