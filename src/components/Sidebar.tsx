...
export default function Sidebar({
  ...
  const providerGroups = PROVIDER_ORDER.map((p) => ({
    provider: p,
    models: filteredModels.filter((m) => m.provider === p),
    total: MIXING_MODELS.filter((m) => m.provider === p).length,
  ...
  const handlePin = async (conv: Conversation) => {
    ...
    try {
      ...
      addToast({ kind: 'error', message: 'PIN FAILED' })
    }
  }
  ...
  const commitRename = async (id: string, value: string) => {
    ...
    try {
      ...
      /* local title already updated optimistically — server sync is best-effort */
    }
  }
  ...
  const handleMsgQueryChange = (q: string) => {
    ...
    msgDebounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        /* search request failed — leave prior results in place */
      }
    ...
    <aside className="rail-left">
      ...
      <div className="sidebar-models-section">
        {mode !== 'image' && (
          <>
            <div className="console-header">
              <h2>Models</h2>
              {blend && (
                <span className="engine-count" style={{ color: 'var(--ok)' }}>
                  BLEND ×{selectedModels.length}
                </span>
              )}
              {selectedModels.length > 0 && (
                <button className="clear-sel-btn" onClick={() => setSelectedModels([])}>
                  ✕ Clear
                </button>
              )}
            </div>
                  ...
                  {models.map((model) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span style={{ color: model.color, fontWeight: 700, fontSize: '11px' }}>
                            {model.initial}
                          </span>
                        </div>
        ...
        {mode === 'image' && (
          ...
          >
            <div style={{ color: 'var(--ink-dim)', fontWeight: 700, marginBottom: '6px' }}>
              IMAGE MODELS
            </div>
            <div>
              Select model + size in the right panel. FLUX.1 Schnell (fast) and FLUX.1 Dev (HD) via
              NVIDIA NIM.
            </div>
          </div>
        )}
      ...
      <div className="sidebar-sessions-section">
        <div className="console-header">
          ...
          {onShowMultiverse && conversations.length > 1 && (
            <button
              className="multiverse-btn"
              onClick={onShowMultiverse}
              title="Open Multiverse (Ctrl+M)"
            >
              🌌
            </button>
          )}
        ...
        {msgSearchOpen && (
          <div className="msg-search-panel">
            <input
              ...
              autoFocus
            />
            {msgSearching && <div className="msg-search-status">searching…</div>}
            ...
            {msgResults.map((r) => (
              ...
              >
                ...
                <div className={`msg-search-role ${r.role}`}>
                  {r.role === 'assistant' ? 'A' : 'U'}
                </div>
                <div className="msg-search-snippet">
        ...
        <div className="session-list">
          ...
          {filtered.map((conv) => (
            ...
            >
              ...
              {conv.pinned && (
                <span className="session-pin-icon" title="Pinned">
                  📌
                </span>
              )}
